...
<? if ($master || $minimaster) { ?>
    ...
    <span class="addbaget"
          style="color:#339; font-weight:700;text-align: left;margin: 10px 15px 0;display: block; cursor:pointer;">Добавление нового багета</span>
    ...
    <div class="newbgt" style="display: none;">
        <div class="maskimg"><img src="/<?= $_COOKIE['calcfile'] ?>" align="left" class="bmenuimg"></div>
        <b class="vendor">Арт. <span class="vendor"><?= $_COOKIE['vendor'] ?></span></b><br>Ширина: <span
                class="width"><?= $_COOKIE['width'] ?></span> мм<br>Без четверти: <span
                class="widthwithout"><?= $_COOKIE['widthwithout'] ?></span> мм<br><span class="pricenew"
                                                                                        style="font-size:125%;"><?= $_COOKIE['pricenew'] ?></span>р.<br>
...
<div style="width:50%; position:relative; float:left; ">Паспарту: <b id="price2">0</b> р.<br>
    ...
    <div class="pasp-block" <? if ($z[3] <> 0) { ?>style="display:block;" <? } ?>>
        <a onclick="z[3]=0; z[4]=0; z[5]=0; changesize(); countprice ();" class="t2 close-pasp-block">Убрать</a><br>Размер
        (мм)<br>
        <form id="form2" name="form2" onsubmit="return false;">
            <input type="text" id="fpasp" name="fpasp" onchange="changesize(); countprice ();" value="<?= $z[5] ?>"
                   autocomplete="off"><br>
            <input type="submit" value="Применить">
        </form>
    </div>
